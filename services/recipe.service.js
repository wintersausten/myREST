...
const getRecipe = async(name) => {
    return Recipe.findOne({name: name}, 
        function(err) {
            if(err) {
                throw new Error(err.message);
            }
...
const updateRecipe = async(name, body) => {
    return Recipe.updateOne({name: name},
        body,
        function(err) {
            if(err) {
                throw new Error(err.message);
...
module.exports = { createRecipe, getAllRecipes, getRecipe, deleteRecipe, updateRecipe };
